...
export const Route = createFileRoute("/$city/gallery")({
  head: ({ params }) => {
    const title = "Media & Gallery — DevKics Abuja";
    const description = "Photos and highlights from DevKics Abuja matchdays at Jabi Astro Turf.";
    return {
      links: [canonicalLink(`/${params.city}/gallery`)],
      meta: seoMeta({
        title,
        description,
        path: `/${params.city}/gallery`,
      }),
    };
  },
  ...
  return (
    <div>
      ...
      <div className="mt-10 space-y-10">
        {galleryQuery.isLoading && <LoadingSkeleton variant="cards" count={3} />}
          ...
          <ErrorState
            ...
            description={
              galleryQuery.error instanceof Error
                ...
                : "An error occurred while fetching media albums."
            }
        ...
        {!galleryQuery.isLoading && !galleryQuery.isError && galleryQuery.data?.length === 0 && (
          <EmptyState
            ...
            description="Matchday photography, tournament highlights, and community snapshots will appear here."
          />
        )}
      </div>
    </div>
